...
def basic_DP_solution(string1,string2, delta, alpha):
    # basic DP solution
    # Def. similarity between two strings 𝑋 and 𝑌 is the minimum cost of an alignment of 𝑋 and 𝑌.
    # X ={X_1, X_2, ... , X_m}
    # Y ={Y_1, Y_2, ... , Y_n}
    # Def. OPT(i, j) is the minimum cost of an alignment of between X_1, X_2, ... , X_i and Y_1, Y_2, ... , Y_j
    ...
    OPT=[]  
    # initialize OPT if i==0 or j==0 in pythonic way
    OPT = [[0 for i in range(len(string2)+1)] for j in range(len(string1)+1)]

    for i in range(1,len(string1)+1):
        for j in range(1,len(string2)+1):

            # sort string1[i-1]+string2[j-1]
            # for example, if string1[i-1] = 'A' and string2[j-1] = 'C', then sort 'AC'
            alpha_key = ''.join(sorted(string1[i-1]+string2[j-1]))
            OPT[i][j] = min(
                OPT[i-1][j-1] + alpha[alpha_key], 
                OPT[i-1][j] + delta, 
                OPT[i][j-1] + delta
                )
    return OPT

# def find_alignment(OPT):

    # find alignment from OPT
    # for 
...
def call_algorithm():
    # Read the input file
    # file = open("SampleTestCases/input0.txt", "r")
    ...
    # basic DP solution
    OPT=basic_DP_solution(generated_string1,generated_string2, delta, alpha)
